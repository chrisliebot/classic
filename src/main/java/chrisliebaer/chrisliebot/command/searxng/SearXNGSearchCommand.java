...
public class SearXNGSearchCommand implements ChrislieListener.Command {
	...
	private static final ErrorOutputBuilder ERROR_EXTERNAL_BANG = ErrorOutputBuilder.generic("Tut mir leid. Bangs und anderen Quatsch unterstütze ich nicht.");
	
	private static final CommonFlex.Provider<SearXNGService.SafeSearch> FLEX_SAFE_SEARCH = CommonFlex.provider(
			(flex, key) -> flex.get(key, SearXNGService.SafeSearch.class), "searx.safeSearch");
	...
	public void execute(Invocation invc) throws ListenerException {
		...
		// check if query is a bang and if so, throw it away
		if (query.startsWith("!") || query.startsWith(":")) {
			ERROR_EXTERNAL_BANG.write(reply).send();
			return;
		}
		...
		Call<SearXNGResult> call = service.search(query, safeSearch, cfg.type, cfg.engines);
	...
	private void printResultItem(ChrislieOutput reply, Result item) {
		...
		StringSubstitutor strSub = new StringSubstitutor(key -> switch (key) {
			...
			case "media" -> (item.imgSrc == null) ? null : (item.imgSrc.startsWith("//")) ? "https:" + item.imgSrc : item.imgSrc;
	...
	private static class Config {
		...
		private @NotNull List<String> engines;
