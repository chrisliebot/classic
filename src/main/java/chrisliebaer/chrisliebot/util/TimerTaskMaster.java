...
/**
 * This class is extending the regular {@link TimerTask} by a commonly found paradigma where an entity wants to create timer tasks but also needs to stop created timer
 * during a shutdown callback. With the regular TimerTask you can't prevent timers from executing their {@link Runnable} after they have been scheduled on their {@link
 * Timer} instance. This class makes all it's timers synchronize on a shared lock while also making sure that each timer is checking a shutdown flag before continuing
